package hr.foka.rezijiser.api.hrt.controller;
...
import hr.foka.rezijiser.api.hrt.service.HrtService;
import hr.foka.rezijiser.persistence.domain.User;

@RestController
@RequestMapping(path = "/e/api/v1/hrt")
public class HrtController {

    private static final Logger LOGGER = LoggerFactory.getLogger(HrtController.class);
    private final HrtService service;
    ...
    public ResponseEntity<?> savePowerBill(@AuthenticationPrincipal User user, @RequestBody HrtResource resource) {
        LOGGER.debug("Received POST request to save power");
        return service.saveResource(user, resource);
    }

    @RequestMapping(method = RequestMethod.PUT)
    public ResponseEntity<?> updatePowerBill(@AuthenticationPrincipal User user, @RequestBody HrtResource resource) {
