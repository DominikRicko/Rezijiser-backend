package hr.foka.rezijiser.api.gas.controller;
...
import hr.foka.rezijiser.api.gas.service.GasService;
import hr.foka.rezijiser.persistence.domain.User;

@RestController
@RequestMapping(path = "/e/api/v1/gas")
public class GasController {

    private static final Logger LOGGER = LoggerFactory.getLogger(GasController.class);
    private final GasService service;

    public GasController(GasService service) {
    ...
    public ResponseEntity<?> savePowerBill(@AuthenticationPrincipal User user, @RequestBody GasResource resource) {
        LOGGER.debug("Received POST request to save power");
        return service.saveResource(user, resource);
    }

    @RequestMapping(method = RequestMethod.PUT)
    public ResponseEntity<?> updatePowerBill(@AuthenticationPrincipal User user, @RequestBody GasResource resource) {
