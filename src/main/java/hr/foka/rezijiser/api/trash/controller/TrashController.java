package hr.foka.rezijiser.api.trash.controller;
...
import hr.foka.rezijiser.api.trash.service.TrashService;
import hr.foka.rezijiser.persistence.domain.User;

@RestController
@RequestMapping(path = "/e/api/v1/trash")
public class TrashController {
    ...
    public ResponseEntity<?> savePowerBill(@AuthenticationPrincipal User user, @RequestBody TrashResource resource) {
        LOGGER.debug("Received POST request to save power");
        return service.saveResource(user, resource);
    }

    @RequestMapping(method = RequestMethod.PUT)
    public ResponseEntity<?> updatePowerBill(@AuthenticationPrincipal User user, @RequestBody TrashResource resource) {
